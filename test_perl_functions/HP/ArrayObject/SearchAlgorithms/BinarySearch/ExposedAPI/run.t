...
my $srchobj = &create_object('c__HP::Array::SearchAlgorithms::BinarySearch__');
...
is ( $result eq 8, 1 );  # Should have defaulted to use linear search due to unsorted array
...
$srchobj->clear();
$srchobj->arrayobject($setobj2->clone());
$srchobj->item($char2find);
$result = $srchobj->run();
diag("Found item <$char2find> at location $result");
is ( $result eq 3, 1 );
