...
$result = &get_from_configuration('way->too->deep2', FALSE);
is ( defined($result), 1 );

is ( ref($result) =~ m/^array/i, 1 );

$result = &get_from_configuration('way->too->deep2', TRUE);
my @result = &get_from_configuration('way->too->deep2', TRUE);
is ( defined($result), 1 );
is ( $result == scalar(@data), 1 );
is ( join(@result) eq join(@data), 1 );
