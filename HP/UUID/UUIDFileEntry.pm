...
BEGIN 
  {
    ...
    $is_init  = 0;
    $is_debug = (
	             $ENV{'debug_capsule_uuidfileentry_pm'} ||
...
sub add
  {
	...
	my $inputdata = {};
    if ( ref($_[0]) !~ m/hash/i ) {
      $inputdata = &convert_input_to_hash([ 'uuid', \&valid_string,
	                                        'filename', \&valid_string ], @_);
    } else {
	  $inputdata = $_[0];
	}

	return FALSE if ( scalar(keys(%{$inputdata})) == 0 );
	return FALSE if ( &equal($inputdata->{'uuid'}, ZERO_UUID) eq TRUE ||
	                  ( not defined($inputdata->{'uuid'})) );
	
	$self->uuid($inputdata->{'uuid'});
...
sub is_valid
  {
    my $self = shift;
	return TRUE if ( ($self->uuid() ne ZERO_UUID) && (defined($self->filename())) );
	return FALSE;
  }
