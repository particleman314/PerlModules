package HP::ExceptionDB::ExceptionType;
...
BEGIN 
  {
    ...
    $is_init  = 0;
    $is_debug = (
                 $ENV{'debug_exceptiondb_exceptiontype_pm'} ||
				 $ENV{'debug_exceptiondb_modules'} ||
...
sub data_types
  {
    ...
    my $data_fields = {
					   ...
					   'id'     => undef,
					   'method' => undef,
					   'obj'    => undef,  # This is a cached version of the base method...
	                  };
...
sub make_exception
  {
	...
	my $classtype = $self->get_exception_class();
	
	if ( not defined($self->obj()) ) {
	  my $result = &is_class_rep('c__'. $classtype .'__');
	  if ( defined($result->{'class'}) ) {
	    $classtype = $result->{'class'};
		$self->get_exception_class($classtype);
	  }
	  $item = &create_object('c__'. $self->get_exception_class() .'__');
	  $self->obj($item);
	} else {
	  $item = $self->obj();
	}
