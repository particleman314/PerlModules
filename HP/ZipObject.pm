...
sub store
  {
    my $self = shift;
	if ( defined($self->compression_type()) ) {
	  my $obj = &create_object($self->compression_type());  # TODO -- Need to map using constant table
	  if ( not defined($obj) ) {
	    &__print_output( 'Unable to generate compression object', FAILURE );
		return FAIL;
	  }
