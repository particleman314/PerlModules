package HP::OOStudio::OO9::XMLRoot;
...
BEGIN 
  {
    ...
    $is_init  = 0;
    $is_debug = (
                 $ENV{'debug_oostudio_oo9_xmlroot_pm'} ||
...
sub post_callback_read
  {
	...
	delete($self->{'node'}) if ( not defined($self->node()) );
	return TRUE;
  }
...
sub write_as_attributes
  {
    ...
    my $specific = [ 'version', 'libraryDocVersion' ];
...
sub validate
  {
    my $self         = shift;
	my $uuidfilename = shift || return FALSE;
	
	return TRUE if ( exists($self->{'node'}) && $self->node()->id() eq "$uuidfilename" );
	return FALSE;
  }
