...
sub data_types
  {
    ...
    my $data_fields = {
					   'rotating'      => FALSE,
			           'system'        => FALSE,
					   'buffer_limits' => $HP::Stream::IOFlags::buffered_size,
			           'maximum_size'  => -1,
		              };
...
sub get_stream_attribute
  {
    &__print_debug_output("Inside 'get_stream_attribute'", __PACKAGE__);
	
    my $self = shift;
	my $attr = shift || return FALSE;
	
	return FALSE if ( ( not exists($self->{"$attr"}) ) );
...
sub hysteresis
  {
    my $self = shift;
	return $HP::Stream::IOFlags::buffered_size;
  }
