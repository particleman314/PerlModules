	  ...
	  if ( defined($self->entry()) && ( &is_type($self->entry(), 'HP::InodeEntry') ) ) {
		...
		if ( $self->entry()->valid() eq TRUE ) {
	      if ( not defined($self->fileglob()) ) {
		    ...
		    # Core of setting up a stream
			my $filepath = $self->entry()->path();
	        my $handle = FileHandle->new("$filepath", 'r');
			if ( defined($handle) ) {
              $handle->autoflush( TRUE );
		      $self->fileglob($handle);
			  $self->active( TRUE );
			} else {
			  &__print_output("Unable to open << $filepath >> for reading...", WARN);
			}
		  }
		} else {
		  $self->valid(FALSE);
