package HP::CSL::Driver::ExecutableOSPath;
...
BEGIN 
  {
    ...
    $is_init  = 0;
    $is_debug = (
                 $ENV{'debug_csl_driver_executableospath_pm'} ||
...
sub __base
  {
    my $self = shift;
	return { 'path' => undef, 'coderef' => FALSE };
  }
...
sub read_xml
  {
	...
	foreach my $sn ( @{$subnodes} ) {
	  my $content = $xmlobj->get_node_content($sn);
	  my $attrs = $xmlobj->get_attributes($sn);
	  
	  my $type = $xmlobj->get_attribute_content({'node' => $sn, 'attribute' => 'type'});
	  next if ( &valid_string($type) eq FALSE );
	  
	  $self->{"$type"} = {};
	  
	  foreach ( @{$attrs} ) {
	    if ( $_ ne 'coderef' ) {
		  $self->{"$type"}->{'path'} = $content;
		  next;
		} else {
		  my $coderef_designation = $xmlobj->get_attribute_content({'node' => $sn, 'attribute' => 'coderef'});
	      $self->{"$type"}->{'coderef'} = &convert_string_to_boolean($coderef_designation);
		}
	  }
	  $self->{"$type"}->{'coderef'} = FALSE if ( not exists($self->{"$type"}->{'coderef'}) );
	}
