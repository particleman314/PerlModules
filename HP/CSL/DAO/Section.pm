...
sub read_xml
  {
	...
	my $rootname  = shift || return $result;
	
	my $xmlobj = &create_object('c__HP::XMLObject__');
	$xmlobj->rootnode($node);
		
	my $properties = $xmlobj->convert_to_properties();
	my $connector = SPLITTER_ARROW;
		
	foreach ( keys(%{$properties}) ) {
	  my @comps = split($connector, "$_");
	  for ( my $loop = 0; $loop < 1; ++$loop ) { shift(@comps); }
	    
	  $properties->{join($connector, @comps)} = $properties->{"$_"};
	  delete($properties->{"$_"});
	}
	
	&save_to_configuration({'table' => $self->configuration(), 'data' => [ $properties ]});
	$result = TRUE;
