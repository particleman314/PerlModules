...
sub validate
  {
	...
	$result = FALSE if ( &valid_string($self->name()) eq FALSE );
	$result = FALSE if ( &valid_string($self->usecase()) eq FALSE && &valid_string($self->workflow()) eq FALSE );
	$result = FALSE if ( &valid_string($self->value()) eq FALSE );
