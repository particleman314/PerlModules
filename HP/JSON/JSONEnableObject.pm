package HP::JSON::JSONEnableObject;
...
BEGIN 
  {
    ...
    $is_init  = 0;
    $is_debug = (
                 $ENV{'debug_json_jsonenableobject_pm'} ||
                 $ENV{'debug_json_modules'} ||
...
sub read_json
  {
	...
	return undef if ( not defined($json_input) );
	
	return decode_json( $json_input );  # Should be a hashref
  }
...
sub write_json
  {
	...
	if ( not defined($hashdata) ) {
	  return encode_json( &as_hash($self) );
	} else {
	  return encode_json( $hashdata );  # Should be a binary UTF-8 string
	}
