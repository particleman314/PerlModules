...
sub display
  {
	...
	$stream->raw_output("\nLock Object ::");
	$stream->raw_output("\tType      --> ". $self->type()) if ( defined($self->type()) );
	$stream->raw_output("\tMechanism --> ". $self->mechanism()) if ( defined($self->mechanism()) );
	$stream->raw_output("\tActive    --> ". &convert_boolean_to_string($self->active()));
	my $data = $self->data();
	
	my @keys = keys(%{$data});
	if ( scalar(@keys) > 0 ) {
	  $stream->raw_output("\tData      ::");
	  foreach ( @keys ) {
	    $stream->raw_output("\t\t$_ --> ". $data->{"$_"});
	  }
	}
	$stream->raw_output("\tKey       --> ". $self->key()) if ( defined($self->key()) );
  }
