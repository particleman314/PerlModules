...
sub add_defines
  {
	...
	return $self->__add_item('defines', $new_define);
  }
...
sub add_targets
  {
	...
	return $self->__add_item('targets', $new_target);
  }
...
sub get_flags
  {
    my $self       = shift;
	my $result     = [];
	my $parameters = $self->flags()->get_elements();
	
	foreach ( @{$parameters} ) {
	  my $flagdef = $_->get_flag();
	  push ( @{$result}, "$flagdef") if ( ( $_->valid eq TRUE ) && 
	                                      ( &valid_string($flagdef) eq TRUE ) );
	}
	
	my $defines = $self->defines()->get_elements();
	
	foreach ( @{$defines} ) {
	  push ( @{$result}, "-D$_" );
	}
	
	push ( @{$result}, @{$self->targets()->get_elements()} );
...
sub remove_defines
  {
	...
	return $self->__remove_item('defines', $old_define);
  }
...
sub remove_targets
  {
	...
	return $self->__remove_item('targets', $old_target);
  }
