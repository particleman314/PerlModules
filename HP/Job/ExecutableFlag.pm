...
sub get_flag
  {
    my $self = shift;
	if ( $self->valid() eq FALSE ) { return undef; }
...
sub set_connector
  {
    my $self = shift;
	my $connector = shift || return;

	&HP::Support::Base::allow_space_as_valid_string(TRUE);
	if ( &valid_string($connector) eq FALSE ) { return; }
...
sub set_name
  {
	...
	if ( &valid_string($name) eq FALSE ) { return; }
...
sub set_value
  {
	...
	if ( &valid_string($value) eq FALSE ) { return; }
...
sub validate
  {
	...
	if ( (( not defined($name) ) && ( not defined($value) )) ||
	     (( not defined($name) ) && defined($value)) ) {
	  $self->valid(FALSE);
	  return;
	}
	
	if ( ( defined($name) && ( not defined($value) )) || 
	     ( defined($name) && ( defined($value) )) ) {
	  $self->valid(TRUE);
	  return;
	}
	return;
