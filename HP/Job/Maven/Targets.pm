...
BEGIN 
  {
    ...
    $is_init  = 0;
    $is_debug = (
                 $ENV{'debug_job_maven_targets_pm'} ||
...
sub add_targets
  {
	...
	$self->targets()->push_item($item) if ( $ref_type eq '' );
	$self->targets()->push_item(${$item}) if ( $ref_type =~ m/^scalar/i );
...
sub get_target
  {
	...
	my $idx = $self->targets()->find_instance($key);
	return undef if ( $idx eq NOT_FOUND );
	return $self->targets()->get_element($idx);
  }
...
sub has_target
  {
	...
	return FALSE if ( $self->targets()->find_instance($key) eq NOT_FOUND );
	return TRUE;
  }
...
sub remove_targets
  {
	...
	return FALSE if ( $self->has_target("$key") eq FALSE );
	return $self->targets()->delete_elements("$key");
  }
