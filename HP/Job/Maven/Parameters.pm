package HP::Job::Maven::Parameters;
...
BEGIN 
  {
	...
	use parent qw(HP::Job::ParameterArray);
    ...
    $is_init  = 0;
    $is_debug = (
                 $ENV{'debug_job_maven_parameters_pm'} ||
...
sub add_parameters
  {
	...
	return if ( ref($item) !~ m/hash/i );
	return if ( not exists($item->{'key'}) );
	return if ( not exists($item->{'value'}) );
	
	$self->parameters()->set_name($item->{'key'});
	$self->parameters()->set_value($item->{'value'});
...
sub remove_parameters
  {
	...
	return FALSE if ( $self->has_parameter("$key") eq FALSE );
	return $self->parameters()->delete_elements($key);
  }
