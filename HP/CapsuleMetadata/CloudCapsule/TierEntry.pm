package HP::CapsuleMetadata::CloudCapsule::TierEntry;
...
BEGIN 
  {
    ...
    $is_init  = 0;
    $is_debug = (
                 $ENV{'debug_capsulemetadata_cloudcapsule_tierentry_pm'} ||
...
sub make_node
  {
    ...
    my $nodename = shift || 'tierelement';

    my $xmlobj  = &create_object('c__HP::XMLObject__');
    my $doctree = $xmlobj->make_document();

    my $node = $doctree->createElement($nodename);
	if ( &valid_string($self->name()) eq TRUE ) {
	  $node->setAttribute('jarname', $self->name());
	  $node->setAttribute('tierid', $self->tierid());
	}
    return $node;
  }
