package HP::CapsuleMetadata::UseCase;
...
BEGIN 
  {
    ...
    $is_init  = 0;
    $is_debug = (
                 $ENV{'debug_capsulemetadata_usecase_pm'} ||
...
sub remove_invalid_entries
  {
	...
	if ( &is_blessed_obj($self->installerData()) eq TRUE ) {
	  $self->installerData()->remove_invalid_entries() if ( &function_exists($self->installerData(), 'remove_valid_entries') eq TRUE );
	}
...
sub write_xml
  {
    my $self     = shift;
	my $filename = shift || &join_path(&getcwd(), 'usecase_capsule.manifest');
