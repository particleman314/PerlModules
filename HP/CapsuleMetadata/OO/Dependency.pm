package HP::CapsuleMetadata::OO::Dependency;
...
BEGIN 
  {
    ...
    $is_init  = 0;
    $is_debug = (
	             $ENV{'debug_capsulemetadata_oo_dependency_pm'} ||
...
sub read_xml
  {
	...
	foreach (@{$attrs}) {
	  my $is_version = ($_ eq 'version') ? TRUE : FALSE;
	  my $data       = $xmlobj->get_attribute_content($node, $_);
	  $self->{$_}    = $data if ( exists($self->{$_}) && $is_version eq FALSE );
	  $self->version()->set_version($data) if ( $is_version eq TRUE );
	}
...
sub write_xml
  {
	...
	my $result = $self->SUPER::write_xml("$filename", 'dep');
	return $result;
  }
