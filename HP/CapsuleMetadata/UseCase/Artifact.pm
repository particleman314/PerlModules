package HP::CapsuleMetadata::UseCase::Artifact;
...
BEGIN 
  {
    ...
    $is_init  = 0;
    $is_debug = (
	             $ENV{'debug_capsulemetadata_usecase_artifact_pm'} ||
...
#  {
#    my $self = shift;
#    my $conversion = { &FORWARD  => [ 'convert_path', 'HP::CapsuleMetadata::UseCase::Tools' ],
#	                   &BACKWARD => [ 'convert_path', 'HP::CapsuleMetadata::UseCase::Tools' ] };
#    my $specific = map { $_ => $conversion } ( 'path' );
...
sub post_callback_read
  {
    my $self = shift;
	my $vobj = $self->version();
	
	$self->version()->update() if ( defined($vobj) );
	delete($self->{'version'}) if ( not defined($self->version()->get_version()) );
	return;
  }
