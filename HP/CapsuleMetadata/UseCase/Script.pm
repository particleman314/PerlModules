package HP::CapsuleMetadata::UseCase::Script;
...
BEGIN 
  {
    ...
    $is_init  = 0;
    $is_debug = (
	             $ENV{'debug_capsulemetadata_usecase_script'} ||
...
sub data_types
  {
    ...
    my $data_fields = {
	                   'path'        => undef,  # Relative to usecase zip file
	                   'name'        => undef,
                       'returncode'  => undef,
					   'lang'        => &LANGUAGE_REFERENCE->{&get_os_type()}->{'shell'},
					   'type'        => PRE_INSTALL_SCRIPT,
		              };
...
sub set_language
  {
	...
	if ( &set_contains( $lang, keys(%{&LANGUAGE_REFERENCE->{&get_os_type()}}) ) eq TRUE ) {
	  $self->lang($lang);
	} else {
	  &__print_output("Unknown language encountered for script!", WARN);
	}
