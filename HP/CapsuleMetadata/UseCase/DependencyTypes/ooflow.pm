package HP::CapsuleMetadata::UseCase::DependencyTypes::ooflow;
...
BEGIN 
  {
    ...
    $is_init  = 0;
    $is_debug = (
	             $ENV{'debug_capsulemetadata_usecase_dependencytypes_modules_ooflow_pm'} ||
	             $ENV{'debug_capsulemetadata_usecase_dependencytypes_modules'} ||
...
sub add_artifact
  {
    my $self     = shift;
	my $artifact = shift || return FALSE;
	
	return FALSE if ( &is_type($artifact, 'HP::CapsuleMetadata::UseCase::Artifact') eq FALSE );
	if ( $self->has_artifact($artifact) eq FALSE ) {
	  $self->artifacts()->push_item($artifact);
	  return TRUE;
	}
...
sub data_types
  {
    ...
    my $data_fields = {
	                   'ooTag'     => undef,
                       'artifacts' => '[] c__HP::CapsuleMetadata::UseCase::Artifact__',
		              };
...
sub has_artifact
  {
	...
	return FALSE if ( &is_type($artifact, 'HP::CapsuleMetadata::UseCase::Artifact') eq FALSE );
	foreach ( $self->artifacts()->get_elements() ) {
	  return TRUE if ( &equal($artifact, $_) eq TRUE );
	}
...
#=============================================================================
sub number_artifacts
  {
    my $self = shift;
	my $artifacts = $self->artifacts();
	
	return 0 if ( not defined($artifacts) );
	return $artifacts->number_elements();
  }
