package HP::Array::SearchAlgorithms::HashSearch;
...
BEGIN 
  {
    ...
    $is_init  = 0;
    $is_debug = (
                 $ENV{'debug_array_searchalgorithms_hashsearch_pm'} ||
...
sub run
  {
	...
	my $elem_type = ( defined($arrobj->type()) ) ? $arrobj->type() : '';
	
	if ( $elem_type eq '' && $item_type eq '' ) {
      %hash = map { $elements[$_] => $_ } 0..$#elements;
	  $result = $hash($item) if ( exists($hash($item)) );
	} else {
