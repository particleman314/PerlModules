package HP::Array::SearchAlgorithms::LinearSearch;
...
BEGIN 
  {
    ...
    $is_init  = 0;
    $is_debug = (
                 $ENV{'debug_array_searchalgorithms_linearsearch_pm'} ||
...
sub run
  {
    my @indices    = ();
    my $self       = $_[0];
	my $inclusive  = $_[1] || $local_false;
	my $good_setup = $self->SUPER::run();
	
	goto __END_OF_SUB if ( $good_setup ne $local_true );

	my @elements = $self->arrayobject()->get_elements();
	my $num_elements = scalar(@elements);
	
	my $item = $self->item();
	
	for ( my $loop = 0; $loop < $num_elements; ++$loop ) {
	  my $result = &equal($elements[$loop], $item);
	  if ( $result eq $local_true ) {
	    CORE::push( @indices, $loop );
	    last if ( $inclusive eq $local_false );
	  }
	}
	
  __END_OF_SUB:
	return NOT_FOUND if ( scalar(@indices) < 1 );
	return @indices  if ( wantarray() );
	return \@indices if ( scalar(@indices) > 1 );
	return $indices[0];
  }
