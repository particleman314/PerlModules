package HP::SupportMatrix::Blueprints;
...
BEGIN 
  {
    ...
    $module_require_list = {
							...
							'HP::SupportMatrix::Blueprint' => undef,
	                       };
    $module_request_list = {};

    $is_init  = 0;
    $is_debug = (
                 $ENV{'debug_supportmatrix_blueprints_pm'} ||
...
sub add_item
  {
    my $self      = shift;
	my $blueprint = shift || return;
	
	if ( not defined($self->{'blueprints'}) ) {
	  $self->blueprints([]);
	} else {
	  if ( ref($self->{'blueprints'}) !~ m/array/i ) {
	    $self->blueprints([]);
	  }
	}
	
	if ( ref($blueprint) =~ m/blueprint/i ) {
	  push ( @{$self->{'blueprints'}}, $blueprint );
	}
  }
...
sub get_number_blueprints
  {
    my $self = shift;
	if ( not defined($self->blueprints()) ) { return -1; }
	return scalar(@{$self->blueprints()});
  }
...
sub parse
  {
	...
	my $specific_nodes = &parse_nodes( $node, 'blueprint', { 'count'   => 0,
										                     'compare' => GEQ, } );
															 
	if ( scalar( @{$specific_nodes} ) > 0 ) {
	  $self->blueprints( [] );
	  foreach ( @{$specific_nodes} ) {
	    my $blueprint = HP::SupportMatrix::Blueprint->new();
        $blueprint->parse( $_ );
	    $self->add_item( $blueprint );
	  }
...
sub verify_data
  {
	...
	my $individual_bps = $self->get_number_blueprints();
