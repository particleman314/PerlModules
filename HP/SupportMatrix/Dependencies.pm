package HP::SupportMatrix::Dependencies;
...
BEGIN 
  {
    ...
    $module_require_list = {
							...
							'HP::SupportMatrix::Dependency' => undef,
	                       };
    $module_request_list = {};

    $is_init  = 0;
    $is_debug = (
                 $ENV{'debug_supportmatrix_dependencies_pm'} ||
...
sub add_item
  {
    my $self   = shift;
	my $dependency = shift || return;
	
	if ( not defined($self->{'dependencies'}) ) {
	  $self->dependencies([]);
	} else {
	  if ( ref($self->{'dependencies'}) !~ m/array/i ) {
	    $self->dependencies([]);
	  }
	}
	
	if ( ref($dependency) =~ m/dependency/i ) {
...
#=============================================================================
sub get_number_dependencies
  {
    my $self = shift;
	if ( not defined($self->dependencies()) ) { return -1; }
	return scalar(@{$self->dependencies()});
  }
...
sub get_dependencies
  {
	...
	my $dependency_objs = $self->dependencies();
	if ( not defined($dependency_objs) ) { return $result; }
	
	foreach ( @{$dependency_objs} ) {
	  push( @{$result}, $_->name() );
	}
...
sub parse
  {
	...
	# Collect Dependency nodes within the Dependencies Node container
	my $specific_nodes = &parse_nodes( $node, 'dependency', { 'count'   => 0,
										                      'compare' => GEQ, } );
	if ( scalar( @{$specific_nodes} ) > 0 ) {
	  # Assign storage as an array
	  $self->dependencies( [] ) if ( not defined($self->{'dependencies'}) );
	  
	  # Loop over all the nodes found
	  foreach ( @{$specific_nodes} ) {
	  
	    # Generate a Dependency Obj and allow it to parse the specific node
	    my $dependency = HP::SupportMatrix::Dependency->new();
        $dependency->parse( $_ );
	    $self->add_item( $dependency );
	  }
	  
	  # Once the Dependencies Node container is completed parsing, mark
	  # duplicate entries.
...
sub verify_data
  {
	...
	my $individual_dependencies = $self->get_number_dependencies();
	
	if ( $individual_dependencies == -1 ) {
	  $self->valid(FALSE);
	  return;
	}
