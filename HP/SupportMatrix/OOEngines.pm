package HP::SupportMatrix::OOEngines;
...
BEGIN 
  {
    ...
    $module_require_list = {
							...
							'HP::SupportMatrix::OOEngine'  => undef,
	                       };
    $module_request_list = {};

    $is_init  = 0;
    $is_debug = (
                 $ENV{'debug_supportmatrix_ooengines_pm'} ||
...
sub add_item
  {
    my $self     = shift;
	my $ooengine = shift || return;
	
	if ( not defined($self->{'ooengine'}) ) {
	  $self->ooengines([]);
	} else {
	  if ( ref($self->{'ooengine'}) !~ m/array/i ) {
	    $self->ooengines([]);
	  }
	}
	
	if ( ref($ooengine) =~ m/ooengine/i ) {
	  push ( @{$self->{'ooengines'}}, $ooengine );
	}
...
sub find_matching_engine
  {
	...
	if ( defined($node) && ref($node) =~ m/libxml/i ) {
	  my $engines = $self->ooengines();
	  foreach ( @{$engines} ) {
	    my $attrnames_prev_engine = $_->attributes();
		my $attrnames_curr_engine = &get_attribute_names( $node );
		
		my $diff_attrs = &set_difference( $attrnames_prev_engine, $attrnames_curr_engine );
		foreach my $atrnm ( @{$attrnames_prev_engine} ) {
		  if ( not defined($attrnames_curr_engine->{"$atrnm"}) ||
		      ( $attrnames_prev_engine->{"$atrnm"} ne $attrnames_curr_engine->{"$atrnm"} ) ) {
		    goto NEW_ENGINE;
	      }
		}
		
		if ( scalar( @{$diff_attrs} ) < 1 ) {
		  $result = $_;
		  last;
		}
	  }
  ...
  NEW_ENGINE:
	if ( not defined($result) ) {
	  $result = HP::SupportMatrix::OOEngine->new();
    }
	return $result;
  }
  
#=============================================================================
sub get_number_ooengines
  {
    my $self = shift;
	if ( not defined($self->ooengines()) ) { return -1; }
	return scalar(@{$self->ooengines()});
  }
...
sub parse
  {
	...
	my $specific_nodes = &parse_nodes( $node, 'ooengine', { 'count'   => 0,
										                    'compare' => GEQ, } );
	if ( scalar( @{$specific_nodes} ) > 0 ) {
	  $self->ooengines( [] );
      foreach ( @{$specific_nodes} ) {
	    my $ooengine = $self->find_matching_engine( $_ );
        $ooengine->parse( $_ );
	    $self->add_item( $ooengine );
	  }
    }
...
sub verify_data
  {
	...
	my $individual_oo = $self->get_number_ooengines();
