package HP::SupportMatrix::Jarfile;
...
BEGIN 
  {
    ...
    $is_init  = 0;
    $is_debug = (
                 $ENV{'debug_supportmatrix_jarfile_pm'} ||
...
sub mark_consistent
  {
	...
	if ( ( $self->name() eq $other->name() && $self->type() ne $other->type() ) || 
	     ( $self->name() ne $other->name() && $self->type() eq $other->type() ) ) {
...
sub verify_data
  {
	...
	if ( not defined($self->name()) || not defined($self->type()) ) {
	  $self->valid(FALSE);
	  return;
	}
	$self->valid(TRUE);
