package HP::SupportMatrix::Dependency;
...
BEGIN 
  {
    ...
    $is_init  = 0;
    $is_debug = (
                 $ENV{'debug_supportmatrix_dependency_pm'} ||
...
sub mark_consistent
  {
	...
	if ( ( $self->name() eq $other->name() && $self->tagID() ne $other->tagID() ) || 
	     ( $self->name() ne $other->name() && $self->tagID() eq $other->tagID() ) ) {
	  &csl_print_debug_output( "Consistency measure for two items FAILED" );
...
sub parse
  {
    ...
    my $tagID = $node->getAttribute('tagID');
    if ( defined($tagID) ) { $self->tagID($tagID); }
...
sub verify_data
  {
	...
	if ( not defined($self->name()) || not defined($self->tagID()) ) {
	  $self->valid(FALSE);
	  return;
	}
	$self->valid(TRUE);
