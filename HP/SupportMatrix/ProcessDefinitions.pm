package HP::SupportMatrix::ProcessDefinitions;
...
BEGIN 
  {
    ...
    $module_require_list = {
							...
							'HP::SupportMatrix::OOEngines'  => undef,
	                       };
    $module_request_list = {};

    $is_init  = 0;
    $is_debug = (
                 $ENV{'debug_supportmatrix_processdefns_pm'} ||
...
#=============================================================================
sub __parse_ooengines
  {
    my $self = shift;
    my $node = shift || return;

    &HP::SupportMatrix::Tools::__parse($self, $node, 'ooengines', { 'type'           => 'multilayer',
										                            'count'          => 0,
										                            'compare'        => GEQ,
										                            'class'          => 'HP::SupportMatrix::OOEngines',
										                            'container_name' => 'ooengines' } );
	return;
  }
...
sub verify_data
  {
	...
	my $ooengines = $self->ooengines();
	if ( defined($ooengines) ) {
	  foreach ( @{$ooengines} ) {
	    if ( $_->valid() == FALSE ) {
		  $self->valid(FALSE);
		  goto FINISH;
