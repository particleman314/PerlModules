...
sub data_types
  {
    ...
    my $data_fields = {
	                   'filename'         => undef,
	                   'injection_points' => 'c__HP::Flare::InjectionPoints__',
...
sub readfile
  {
    ...
    return $result if ( &valid_string($filename) eq FALSE );
	$self->filename(&normalize_path($filename));
	return $self->SUPER::readfile("$filename");
  }
